...
@synthesize radialDistance = _radialDistance;
...
{	
	__autoreleasing ARCoordinate *newCoordinate	= [[ARCoordinate alloc] init];
...
- (BOOL)isEqualToCoordinate:(ARCoordinate *)otherCoordinate
{
	...
	if ((self.title && otherCoordinate.title) || (self.title && !otherCoordinate.title) ||
...
{
	return [NSString stringWithFormat:@"%@ r: %.3fm φ: %.3f° θ: %.3f°", self.title, self.radialDistance,
            radiansToDegrees(self.azimuth), radiansToDegrees(self.inclination)];
}
